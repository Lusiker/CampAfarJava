...
CREATE TABLE `question` (
  ...
  `question_last_modified` timestamp NULL DEFAULT NULL, #提问上次编辑时间
  ...
  `question_is_free` tinyint(1) NOT NULL default '0', #问题是否免费， 在回答创建时需要填写
  `question_listen_price` decimal(7, 2) NOT NULL default '0.00', #旁听费用， 在回答创建时需要填写
  ...
  `answer_last_modifier` timestamp NULL DEFAULT NULL, #回答上次编辑时间
