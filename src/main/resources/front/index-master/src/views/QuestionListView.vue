...
<!--          </div>-->
    ...
    <div class="grey"></div>
    <div class="box-body" @click="goToquestionDetails">
      <div class="" v-for="item in questionlist" :key="item.id">
        <div class="item" @click.stop="toDetail(item)">
          <div class="item-comp">
            <div class="text">
              <div class="text-title">{{ item.head }}</div>
              <div class="text-info">

                <div class="text-info-left">{{"发起方:"+item.uidFrom}}</div>
                <div class="text-info-right">{{"接收方:"+item.uidTo}}</div>
    ...
    <kp-foot-nav type="question"></kp-foot-nav>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      isShow:false,
      questionlist:[
        {head:'前后端如何数据联调',uidFrom:'1',uidTo:'2'},
        {head:'vue生命周期的讲解',uidFrom:'3',uidTo:'4'},
      ],
  ...
  methods: {
    ...
    goToquestionDetails: function (item) {
      //this.$router.push({
      // path: "/vod/detail",
      // query: {
      //   id: item.id,
